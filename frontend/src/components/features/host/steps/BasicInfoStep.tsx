...
interface BasicInfoStepProps {
  ...
  setCreatedPropertyId: (id: number) => void
}

const BasicInfoStep: React.FC<BasicInfoStepProps> = ({ setLoading, setCreatedPropertyId }) => {
  const context = useContext(WizardContext)
  if (!context) throw new Error('BasicInfoStep must be used within WizardContext')
  ...
  // Improved number input handlers
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        alert('Lỗi khi tạo property: ' + (response.message || 'Unknown error'))
      }
    } catch (error) {
      console.error('Error creating property:', error)
      alert('Lỗi khi tạo property')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6 px-4 sm:px-0">
      <div className="text-center">
        ...
        <p className="text-gray-600 text-sm sm:text-base">Nhập thông tin cơ bản về property của bạn</p>
      </div>
      ...
      <div className="flex justify-end pt-6">
        <button
          ...
          className="w-full sm:w-auto px-6 sm:px-8 py-3 bg-gradient-to-r from-pink-600 to-purple-600 text-white rounded-lg hover:from-pink-700 hover:to-purple-700 transition-all flex items-center justify-center font-medium shadow-lg text-sm sm:text-base"
        >
          Tạo Property
